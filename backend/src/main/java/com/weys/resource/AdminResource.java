...
import com.weys.service.HeliusService;
...
public class AdminResource {
    ...
    public record Stats(long totalWallets, long totalPoints, long weyHolders,
            long betaCodes, long betaCodesRedeemed, long weysHeld,
            long walletsWithWeys, long dbMarketCap, long floorPrice) {
    }
    ...
    public Stats getStats(@HeaderParam("X-Public-Key") String wallet) {
        ...
        long totalPoints = User.streamAll()
                ...
                .sum();
        long weyHolders = User.count("weyHolder", true);
        long betaCodes = BetaCode.count();
        long betaCodesRedeemed = BetaCode.count("redeemed", true);

        long weysHeld = 0;
        long walletsWithWeys = 0;
        String heliusKey = System.getenv("REACT_APP_HELIUS_API_KEY");
        String collection = System.getenv().getOrDefault("REACT_APP_WEYS_COLLECTION", "primos");
        if (heliusKey != null && !heliusKey.isEmpty()) {
            List<User> users = User.listAll();
            for (User u : users) {
                int count = fetchWeyCount(u.getPublicKey(), heliusKey, collection);
                if (count > 0) {
                    walletsWithWeys++;
                    weysHeld += count;
                }
        ...
        long floorPrice = fetchFloorPrice(collection);
        long dbMarketCap = weysHeld * floorPrice;

        return new Stats(totalWallets, totalPoints, weyHolders, betaCodes,
                betaCodesRedeemed, weysHeld, walletsWithWeys, dbMarketCap, floorPrice);
    }

    private int fetchWeyCount(String wallet, String apiKey, String collection) {
        try {
            int page = 1;
            int limit = 100;
            int total = 0;
    ...
    private long fetchFloorPrice(String collection) {
        try {
            String url = String.format("https://api-mainnet.magiceden.dev/v2/collections/%s/stats", collection);
            HttpRequest req = HttpRequest.newBuilder()
                    .uri(URI.create(url))
                    .GET()
                    .build();
    ...
    /**
     * Discovers tokens held by Wey holders and adds them to Trenches.
     * This endpoint fetches all tokens held by Wey community members
     * and automatically adds popular tokens to the Trenches for community tracking.
     */
    @POST
    @Path("/discover-wey-tokens")
    public String discoverWeyTokens(@HeaderParam("X-Public-Key") String wallet) {
        ensureAdmin(wallet);
        return weyTokensService.discoverAndAddWeyTokens();
    }
}
