...
import com.weys.resource.AdminResource;
...
public class LoginService {
    ...
    public User login(LoginRequest req) {
        ...
        if (user == null) {
            ...
            // If beta code was provided, treat user as wey holder
            ...
            if (!user.isBetaRedeemed() && req.betaCode != null && !req.betaCode.isEmpty()) {
                // Redeemed a beta code: grant betaRedeemed, weyHolder, and store code
                validateBetaCodeOrThrow(req.betaCode);
                user.setBetaRedeemed(true);
                user.setWeyHolder(true);
                user.setBetaCode(req.betaCode);
            }
            if (LOGGER.isLoggable(java.util.logging.Level.INFO)) {
        ...
        boolean wasHolder = user.isWeyHolder();

        // Update NFT count from the blockchain on each login
        int count = heliusService.getWeyCount(user.getPublicKey());
        ...
        if (LOGGER.isLoggable(java.util.logging.Level.INFO)) {
            if (user.isWeyHolder()) {
                LOGGER.info(String.format("[LoginService] Wey holder login for publicKey: %s", req.publicKey));
            } else {
                LOGGER.info(String.format("[LoginService] Non-wey login for publicKey: %s", req.publicKey));
            }
        }
