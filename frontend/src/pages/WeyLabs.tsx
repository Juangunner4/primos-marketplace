...
const WeyLabs: React.FC<{ connected?: boolean }> = ({ connected }) => {
  const wallet = useWallet();
  const { isHolder } = useWeyHolder();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const membersRes = await api.get<Member[]>('/api/user/weys', {
